CREATE FUNCTION `GET_CHILDREN_PERMISSION_IDS`(rootId VARCHAR(100))
...
BEGIN
    ...
    WHILE childrenIds is not null DO
            ...
            SELECT group_concat(id) INTO childrenIds FROM `permission` where FIND_IN_SET(parent_id, childrenIds);
        END WHILE;
